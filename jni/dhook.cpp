...
void copyJumpToTargetBytes(byte * bytes, p_void target) {
    for(int i = 0; i < 4; i++) {
...
p_void DHookTrampoline(p_void origin, p_void detour) {
    ...
    copyJumpToTargetBytes((byte*) dhc->branchDetourBytes, detour);
    ...
    uint32_t returnAddress = ((uint32_t)origin + 0x8); // This is were we want to return after we finish with our detour code 

    // dhc->restorationBytes contain the original function bytes in the first 8 bytes, the remaining 8 bytes are used to jump back to origin+0x8
    copyJumpToTargetBytes((byte*)((uint32_t)dhc->restorationBytes + 8), (p_void)returnAddress); 
